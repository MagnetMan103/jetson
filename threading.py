...
class LatestFrameCapture:
    """
    Continuously captures frames in background thread and provides latest frame.
    This prevents processing stale frames from the camera buffer during YOLO processing.
    """
    ...
    def __init__(self, camera):
        ...
        if self.frame is None:
            raise RuntimeError("Failed to capture initial frame from camera")
        
        print("Frame capture thread started successfully!")
    ...
    def get_latest_frame(self):
        """
        Returns the most recent frame, discarding any older buffered frames.
        This ensures YOLO always processes fresh data captured AFTER robot movements.
        """
        with self.lock:
...
class HexapodController:
    """Manages serial communication with Arduino for hexapod control"""
    ...
    def __init__(self, port='/dev/ttyACM0', baudrate=115200):
        ...
        # Background reading control
        self.pause_reading = False
        self.running = True
        
        # Start background thread to read IMU data
        self.read_thread = threading.Thread(target=self._read_loop, daemon=True)
        self.read_thread.start()
        
        print(f"{Colors.GREEN}Arduino connected!{Colors.RESET}")
        time.sleep(0.5)
    ...
    def send_motor_command(self, direction, amplitude):
        """
        Send a motor command to the Arduino and wait for completion
...
def main():
    ...
    print(f"{Colors.GREEN}Hexapod Bottle Chase Controller (Threaded Frame Capture){Colors.RESET}")
    print(f"{Colors.GREEN}{'='*70}{Colors.RESET}\n")
    ...
    try:
        ...
        print(f"\n{Colors.CYAN}Starting threaded frame capture...{Colors.RESET}")
        frame_grabber = LatestFrameCapture(cap)
        print(f"{Colors.GREEN}Threaded capture active - always using latest frames!{Colors.RESET}\n")
        ...
        print(f"{Colors.CYAN}Threaded capture ensures YOLO always processes fresh frames{Colors.RESET}")
        print(f"{Colors.CYAN}Frames captured DURING YOLO processing are discarded automatically{Colors.RESET}\n")
        
        frames_without_bottle = 0
        max_frames_without_bottle = 5
        
        while running:
            # STEP 1: Get the LATEST frame (discards stale buffered frames)
            # This frame was captured recently, not 200ms ago!
            ret, frame = frame_grabber.get_latest_frame()
            if not ret or frame is None:
                print("Failed to grab frame")
                time.sleep(0.1)
                continue
            
            # STEP 2: Run detection on this FRESH frame
            # While YOLO runs (200ms), background thread continues capturing new frames
            ...
            if bottles:
                ...
                # Turn to face the bottle if needed
                ...
                if needs_recentering:
                    ...
                    print(f"{Colors.GREEN}Recenter complete. Next iteration will use fresh frame!{Colors.RESET}\n")
                    
                    # Continue to next iteration
                    # Background thread has been capturing frames during the turn
                    # Next get_latest_frame() will return a POST-turn frame
                    continue
                    ...
                    print(f"{Colors.GREEN}Move complete. Next iteration will use fresh frame!{Colors.RESET}\n")
                    
                    # Continue to next iteration
                    # Background thread has been capturing during movement
                    # Next get_latest_frame() will return a POST-movement frame
                    continue
